...
{   
    float JitterNoise = GetBNDSequenceSample(pixCoord.xy, _HFrameCount, 0);
...
{
    Traversal.Normal = step(Traversal.Max.xyz, Traversal.Max.zxy) * step(Traversal.Max.xyz, Traversal.Max.yzx); //实时计算法线，而不是冲体素数据中获取
...
//HDDA 光线步进算法  即分层光线步进   和   GDC2023  的 HSGI算法类似
...
bool TraceVoxelsDiffuse(float3 RayOrigin, float3 RayDirection, float TraceDistance, uint MaxIterations, inout VoxelPayload Payload)
{
	...
	Traversal.Coord = 0;
	Traversal.Dist = 0;
    Traversal.CurrentClipmapLevel = 0;
			...
			while (Traversal.Dist < TraceDistance && i++ < 4)
			{
				...
				{	
					...
					if ((BitMask & VoxelOccupancy) != 0u)
					{
						...
						Payload.HitDistance = Traversal.Dist;
                        Payload.HitNormal = normalize(-Traversal.Normal * Traversal.DirSign); //实时计算法线，而不是冲体素数据中获取
