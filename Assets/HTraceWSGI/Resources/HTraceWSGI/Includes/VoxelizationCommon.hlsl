...
// Clipmap parameters
int _EnableClipmaps;
float3 _VoxelizationClipmapAABB_Min;
float3 _VoxelizationClipmapAABB_Max;
float3 _VoxelClipmapResolution;
float3 _VoxelClipmapBounds;
float _VoxelClipmapPerMeter;
float _VoxelClipmapSize;

H_RW_TEXTURE3D(uint, _VoxelData);
H_TEXTURE3D(uint, _VoxelPositionPyramid);

// Clipmap textures
H_RW_TEXTURE3D(uint, _VoxelClipmapData);
H_TEXTURE3D(uint, _VoxelClipmapPositionPyramid);
