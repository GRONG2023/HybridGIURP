...
MaterialFloat4 ProcessMaterialColorTextureLookup(MaterialFloat4 TextureValue)
{
	...
	TextureValue.rgb = Gamma22ToLinear(TextureValue.rgb);
#endif
...
MaterialFloat4 Texture2DSampleGrad( Texture2D Tex, SamplerState Sampler, float2 UV, MaterialFloat2 ioDDX, MaterialFloat2 ioDDY )
{
	UV.y = 1.0 - UV.y;
	#if HDRP || URP
		return SAMPLE_TEXTURE2D_GRAD( Tex, Sampler, UV, ioDDX, ioDDY );
	#else
		return tex2Dgrad( Tex, UV, ioDDX, ioDDY );
	#endif
}
...
MaterialFloat4 Texture2DSampleBias( Texture2D Tex, SamplerState Sampler, float2 UV, float MipBias )
{
	...
	MaterialFloat4 Texture2DArraySampleGrad( Texture2DArray Tex, SamplerState Sampler, float3 UV, MaterialFloat2 ioDDX, MaterialFloat2 ioDDY )
	{
		UV.y = 1.0 - UV.y;
		return SAMPLE_TEXTURE2D_ARRAY_GRAD( Tex, Sampler, UV.xy, UV.z, ioDDX, ioDDY );
	}
	MaterialFloat4 Texture2DSampleGrad( Texture2DArray Tex, SamplerState Sampler, float3 UV, MaterialFloat2 ioDDX, MaterialFloat2 ioDDY )
	{
		return Texture2DArraySampleGrad( Tex, Sampler, UV, ioDDX, ioDDY );
	}
...
{
	float2 PixelPos = BufferUV.xy * View_BufferSizeAndInvSize.xy;
...
float2 GetPixelPosition( FMaterialPixelParameters Parameters )
{
	return Parameters.SvPosition.xy;
}
...
float2 ViewportUVToBufferUV( float2 ViewportUV )
{
	...
	return PixelPos * View_BufferSizeAndInvSize.zw;
}
